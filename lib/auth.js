...
const REQUIRED_KEYS = ['DATABASE_URL','STACK_AUTH_PROJECT_ID','STACK_AUTH_CLIENT_ID','STACK_AUTH_CLIENT_SECRET','SESSION_SECRET'];
...
async function verifyToken(token) {
  ...
  if (JWT_SECRET) {
    try { const { payload } = await jwtVerify(token, await getSecret()); return payload; } catch {}
  }
  if (JWKS_URL) {
    try { const { payload } = await jwtVerify(token, await getJwks()); return payload; }
    catch (err) { console.error('verifyToken error:', err.message); return null; }
  }
  return null;
}
